...
private fun TemplateEditorListItem(
    ...
    val preview = remember(value, multiline) {
        if (multiline) {
            val lines = value.lines()
            if (lines.isEmpty() || value.isBlank()) {
                "—"
            } else {
                val previewText = lines.take(4).joinToString("\n")
                if (lines.size > 4) {
                    "$previewText\n…"
                } else {
                    previewText
    ...
    ExpressiveListItem(
        ...
        onClick = { if (!readOnly) showDialog.value = true }
    )
}
